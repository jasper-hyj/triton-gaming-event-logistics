    ...
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: '⚙️ Condition', value: item.condition || 'Unknown', inline: true },

          // ─────── Contents ───────
          { name: '\u200B', value: '**📦 Contents**', inline: false },
          { name: '🔌 Ports', value: formatArray(item.ports), inline: true },
          { name: '🧩 Parts Included', value: formatArray(item.parts), inline: true },
          { name: '❌ Missing Parts', value: formatArray(item.missings), inline: true },

          // ─────── Software ───────
          { name: '\u200B', value: '**💻 Software / Access**', inline: false },
          { name: '📂 Installations', value: formatArray(item.installations), inline: true },
          { name: '🔐 Password', value: item.password || '_None_', inline: true },

          // ─────── Metadata ───────
          { name: '\u200B', value: '**📝 Metadata**', inline: false },
          { name: '📦 Source', value: item.source || '_Unknown_', inline: true },
          { name: '🏷️ Provider', value: item.provider || '_Unknown_', inline: true },
          { name: '\u200B 🗒️ Note', value: item.note || '_None_', inline: false }
        )
        .setFooter({
